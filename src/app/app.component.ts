...
export class AppComponent {
  ...
  ngOnInit() {
    this.subs = this.http.get('assets/sample-input.txt', { responseType: 'text' }).subscribe( {
       next: (data: any) => {
  ...
  parseFileData = (fileData: string) => {
     ...
     parsedCodes.map((code: string )=> { 
        ...
        const parsedNumber = "" + parseInt(code[firstDigitIndex][0], 10) + parseInt(reversedCode[lastDigitIndex][0], 10)
        parsedNumbers.push(parseInt(parsedNumber));
     });
     this.summedValue = parsedNumbers.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
  }
